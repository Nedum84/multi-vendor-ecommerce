...
import userFake from "./test.faker";
...
describe("User Tests...", () => {
  ...
  it("Can update password", async () => {
    ...
    //Reg User
    const { body } = await customRequest({ path: `/auth`, method: "post", payload: create });
  ...
  it("Can find my detail", async () => {
    const response = await customRequest(`/user/me`);
  ...
  it("Can find a user by ID", async () => {
    ...
    const response = await customRequest(`/user/${user_id}`);
  ...
  it("Can find a user by email", async () => {
    ...
    const response = await customRequest(`/user/email/${email}`);

    expectSuccess(response);
  });

  it("Can all users", async () => {
    const response = await customRequest(`/user`);
