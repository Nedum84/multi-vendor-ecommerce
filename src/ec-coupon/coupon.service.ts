...
import CouponUtils from "./coupon.utils";
...
const create = async (req: Request) => {
  ...
  if (coupon_apply_for == CouponApplyFor.PRODUCT && body.products.length == 0) {
    throw new BadRequestError("category(category_id) is required for CATEGORY COUPON TYPE");
  }

  if (coupon_apply_for == CouponApplyFor.STORE && body.stores.length == 0) {
    throw new BadRequestError("store(store_id) is required for STORE COUPON TYPE");
  }

  if (coupon_apply_for == CouponApplyFor.USER && body.users.length == 0) {
    throw new BadRequestError("user(user_id) is required for USER COUPON TYPE");
  }
  ...
  if (
    ...
    throw new BadRequestError("users & product are required for USER_AND_PRODUCT COUPON TYPE");
  }
  if (coupon_type !== CouponType.PERCENTAGE) {
    throw new BadRequestError("Only percentage discount is available at the moment");
  }
  ...
  if (storeList.includes(coupon_apply_for) && !isAdmin(role)) {
    if (coupon_apply_for === CouponApplyFor.STORE) {
      ...
      if (body.stores.length > 1) {
        throw new BadRequestError("Store can only create one STORE COUPON TYPE");
      }
...
const applyCoupon = async (user_id: string, coupon_code: string) => {
    ...
    case CouponApplyFor.PRODUCT:
      ...
      carts.forEach((cart) => {
        ...
        if (couponProductIds.includes(product_id)) {
          ...
          couponAmount += CouponUtils.calcCouponAmount(
            coupon,
            qty,
            price,
            discount,
  ...
  if (couponAmount == 0) {
    throw new BadRequestError("Coupon not available for this user/product/orders");
  }
    ...
    carts.forEach((cart) => {
      ...
      if (couponStoreIds.includes(store_id)) {
        ...
        if (each_store_id === store_id) {
          ...
          // }
          storeCouponAmount += CouponUtils.calcCouponAmount(
            coupon,
            qty,
            price,
            discount,
