...
import sequelize, { Category, Product } from "../ec-models";
import { ProductDiscountAttributes } from "../ec-product-variation/product.discount.model";
import { ProductAttributes, ProductInstance } from "./product.model";
import ProductUtils from "./product.utils";
import { createModel, genSlugColId, getPaginate } from "../ec-models/utils";
import categoryService from "../ec-category/category.service";
import { ProductVariationAttributes } from "../ec-product-variation/product.variation.model";
...
import collectionService from "../ec-collection/collection.service";
import { generateSlug, mapAsync } from "../ec-utils/function.utils";
...
import { Paginate } from "../ec-models/types";
...
const create = async (req: Request) => {
  ...
  if (!isAdmin(role) && role != UserRoleStatus.VENDOR) {
    throw new ForbiddenError("Only vendore/Admin can create a product");
  }

  //not necessary validation though🤪
  if (body.variation.with_storehouse_management) {
    if (!body.variation.stock_qty) {
      throw new BadRequestError("Quantity is required for store house management");
    }
  } else {
    if (!body.variation.stock_status) {
      throw new BadRequestError("Stock status is required for store house management");
    }
  }

  if (body.discount) {
    if (body.discount.price > body.variation.price) {
      throw new BadRequestError("Discount cannot be less than the actual price");
    }
  }
  ...
  try {
    await sequelize.transaction(async (transaction) => {
      ...
      const { product_id } = await createModel<ProductInstance>(Product, body, "product_id", {
        transaction,
      });
...
const findAll = async (req: Request) => {
  const { store_id, category_id, collection_id, search_query, is_approved, is_featured } =
    req.query as any;
  const options = getPaginate(req.query);
