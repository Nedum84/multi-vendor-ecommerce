...
import creditCodeFake from "./credit.code.fake";
...
describe("Credit Code Tests", () => {
  ...
  it("Can check if credit_code exist", async () => {
    ...
    const creditCodeExistResponse = await customRequest({
      path: `/credit/check-exist`,
      method: "post",
  ...
  it("Can find credit by credit_code", async () => {
    ...
    const response = await customRequest(`/credit/${credit_code}`);
    expectSuccess(response);
  ...
  it("Can find all", async () => {
    ...
    const response = await customRequest(
      `/credit?credit_type=${CreditCodeType.USER}&search_query=${payload.title}`
    );
